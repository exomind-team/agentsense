...
pub async fn fetch(
    ...
    let usage_data = usage
        .data
        .ok_or_else(|| AgentSenseError::Http("MiMo usage: empty data".into()))?;
    let month = usage_data
        .month_usage
        .ok_or_else(|| AgentSenseError::Http("MiMo usage: no month_usage".into()))?;
    let item = month
        .items
        .and_then(|items| items.into_iter().next())
        .ok_or_else(|| AgentSenseError::Http("MiMo usage: no items".into()))?;
