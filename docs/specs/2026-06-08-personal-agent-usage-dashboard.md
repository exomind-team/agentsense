...
- 工作区消耗 Top 这类密集表格已经改为完整宽度承载，长路径和记录在单元格内处理，窄屏只在表格容器内滚动，避免右侧“来源/记录”列被裁掉。
...
结构化呈现不是把所有 widget 均匀塞进网格。排行表、来源表、工作区表这类字段密度高的 widget 应该按信息密度选择承载宽度:

- 模型、工作区、来源状态这类表格默认独占一行，避免长路径、记录摘要、来源徽标在半宽容器里互相挤压。
- 表格可以固定列宽并在单元格内部处理长文本: 路径省略并保留 `title`，记录列允许换行，来源徽标允许换行堆叠。
- 窄屏时横向滚动只出现在表格容器内部，页面整体不能被撑宽，也不能用 `overflow: hidden` 直接裁掉右侧列。
- 指标卡、短摘要、小型状态块可以并排；趋势图和密集表格应该优先获得可读面积。
