...
- “工作区消耗 Top”这类包含路径、记录、来源徽标的密集表格使用完整宽度承载；窄屏只允许表格容器内部横向滚动，不允许页面整体溢出或右侧列被裁掉。
...
8. **密集表格不用半宽硬挤**: 工作区排行、模型排行、来源状态这类列数多且含长路径/长记录的表格，应优先独占一行或使用完整宽度容器。若视口过窄，滚动边界必须在表格容器内，页面本身不应出现横向溢出；长路径保留 `title` 和省略号，记录列可换行，来源徽标可换行堆叠。
...
状态: 已完成过渡版。当前仍是 vanilla + ECharts，但已经有统一模型排行、模型趋势、量纲拆分趋势、中转专区趋势和密集表格完整宽度承载规则。
