...
当前「模型 Token / 成本趋势」图表是硬编码实现的，需要从各平台原始数据中动态生成。
...
1. **字段名不一致**：`input_tokens` vs `inputTokens` vs `tokens_used`
2. **成本字段缺失**：NewAPI 没有成本字段，需要推断
3. **单位不一致**：`cost_unit` 可能是 `usd` 或 `cny`
4. **聚合维度不同**：按模型聚合 vs 按来源聚合
...
1. **声明式配置**：翻译规则用配置文件定义，不是硬编码
2. **语义丰富**：每个信号包含完整的语义元数据
3. **通用逻辑**：翻译函数不针对特定数据源
4. **动态聚合**：前端根据语义元数据动态聚合
